...
(
  set -x
  find "$base"/src/main -type f -exec sed -i s/housetils/"$modid"/g\;s/live.ixnoah.housetils/"$package_name"/g {} +
  sed -i s/live.ixnoah.housetils/"$package_name"/g\;s/housetils/"$modid"/g "$base"/gradle.properties
  sed -i s/housetils/"$project_name"/g "$base"/settings.gradle.kts
  ...
  mv "$base"/src/main/resources/mixins.housetils.json "$base"/src/main/resources/mixins."$modid".json
